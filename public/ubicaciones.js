...
async function listar(){
  ...
  const identificacion = $('fIdentificacion')?.value?.trim();
  const sede = $('fSede')?.value?.trim();
  const edificio = $('fEdificio')?.value?.trim();
  const piso = $('fPiso')?.value?.trim();
...
document.getElementById('btnLimpiar')?.addEventListener('click', () => {
  ...
  ['q', 'fIdentificacion', 'fSede', 'fEdificio', 'fPiso'].forEach(id => {
    const input = $(id);
