...
// Permite admin o administrativo
const requireAdminOrAdminist = (req, res, next) => {
  if (!req.user) return res.redirect('/login');
...
const requireOnlyAdmin = (req, res, next) => {
  if (!req.user || req.user.role !== 'admin') {
    return res.status(403).send('Acceso prohibido - Solo administrador');
  }
  next();
};
...
// API completa de equipos con filtros (reemplaza o complementa la vista actual)
app.get('/api/equipos', requireAuth, async (req, res) => {
  try {
    const { q, estado, tipo_equipo, ubicacionId, responsableId, marca, modelo } = req.query;
    
    const where = {};

    if (q) {
      where[Op.or] = [
        { codigo_inventario: { [Op.like]: `%${q}%` } },
        { serial:            { [Op.like]: `%${q}%` } },
        { marca:             { [Op.like]: `%${q}%` } },
        { modelo:            { [Op.like]: `%${q}%` } }
      ];
...
// 1️⃣ Listar todas (con filtros opcionales)
app.get('/api/ubicaciones', requireAuth, async (req, res) => {
  try {
    const { q, identificacion, sede, edificio, piso } = req.query;
    const where = {};
    
    // Filtro general (busca en múltiples campos)
    if (q) {
      where[Op.or] = [
        { identificacion: { [Op.like]: `%${q}%` } },
        { sede:           { [Op.like]: `%${q}%` } },
    ...
    if (edificio) where.edificio = { [Op.like]: `%${edificio}%` };
    if (piso) where.piso = { [Op.like]: `%${piso}%` };
...
// 2️⃣ Obtener una ubicación por ID
 ...
 * =============================== */
/* ===============================
 *  APIs — Responsables/Custodios (con filtros)
 * =============================== */
app.get('/api/responsables-custodios', requireAuth, async (req, res) => {
  try {
    const { q, id_area } = req.query;
    ...
    if (id_area) where.id_area = { [Op.like]: `%${id_area}%` };

    const items = await ResponsableCustodio.findAll({
      where,
      order: [['id_area', 'ASC']]
    });
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user.role === 'admin') {
      res.redirect('/');  // Admin → home.ejs (dashboard completo)
    } else {
      res.redirect('/');  // User → homeuser.ejs (vista limitada)
    }
  } catch (e) {
    console.error(e);
...
app.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const operativos = await Equipo.count({ where: { estado: 'operativo' } });
    const enMantenimiento = await Equipo.count({ where: { estado: 'en_mantenimiento' } });
    const dadosDeBaja = await Equipo.count({ where: { estado: 'dado_de_baja' } });

    // === CONTEOS DE MANTENIMIENTOS ===
    const hoy = new Date();
    ...
    const mantenimientosPendientes = await Mantenimiento.count({
      where: {
        resultado: null,
        fecha_programada: { [Op.lte]: hoy }
      }
    });

    const proximosAVencer = await Mantenimiento.count({
      where: {
        resultado: null,
        proximo_vencimiento: {
          [Op.gte]: hoy,
          [Op.lte]: new Date(hoy.getTime() + 30 * 24 * 60 * 60 * 1000)
        }
      }
    });

    const realizadosEsteMes = await Mantenimiento.count({
      where: {
        fecha_ejecucion: {
          [Op.gte]: new Date(hoy.getFullYear(), hoy.getMonth(), 1),
          [Op.lte]: hoy
        },
        resultado: { [Op.ne]: null }
      }
    });
...
// Ruta para usuarios normales (role: 'user')
app.get('/homeuser', requireAuth, async (req, res) => {
  // Verificar que el usuario no sea admin
  if (req.user.role === 'admin') {
    return res.redirect('/'); // Si es admin, enviarlo al dashboard completo
  }

  try {
    // Conteos básicos para mostrar al usuario normal (opcional, tu vista no los usa)
    const totalEquipos = await Equipo.count();
    const operativos = await Equipo.count({ where: { estado: 'operativo' } });
    const enMantenimiento = await Equipo.count({ where: { estado: 'en_mantenimiento' } });
    ...
    console.error('Error en homeuser:', err);
    res.render('homeuser', { stats: null });
  }
});
